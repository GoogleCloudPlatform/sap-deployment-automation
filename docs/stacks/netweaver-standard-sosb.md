...
| `sap_zone` | The zone for sap HANA and Netweaver instances, for example `us-central1-a`. | `string` |
...
| `sap_hana_instance_count_worker` |  Hana worker instance count to deploy | `integer` | 1 | no |
| `sap_hana_instance_count_standby` |  Hana standby instance count to deploy. Currently the stack only supports one standby node deployment. Will update the section here when the code to support multiple standby nodes is provided | `integer` | 1 | no |
